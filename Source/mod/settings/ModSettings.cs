...
namespace DarkIntentionsWoohoo.mod.settings
{
    ...
    class WoohooMod : Mod
    {
        ...
        {
            this.settings.woohooChildChance = Widgets.HorizontalSlider(inRect.TopHalf().TopHalf(), this.settings.woohooChildChance, 0f, 1f, true, "Risky Woohoo Factor " + this.settings.woohooChildChance + "\nDetermines How likely do we roll for pregnancy", "0%", "100%");
            this.settings.woohooBabyChildChance = Widgets.HorizontalSlider(inRect.TopHalf().BottomHalf(), this.settings.woohooBabyChildChance, 0f, 1f, true, "How Likely We Are To Try and Get Pregnant why Trying For Baby : " + this.settings.woohooBabyChildChance + "\n. The base game forces a second dice role on your pawns age group. Fertility Starts at 14 in base game.", "0%", "100%");
            this.settings.sameGender = Widgets.HorizontalSlider(inRect.TopHalf().BottomHalf(), (this.settings.sameGender?1f:0f), 0f, 1f, true, "Goodwill effect on success: " + this.settings.sameGender, "No", "Yes")>.5f;
